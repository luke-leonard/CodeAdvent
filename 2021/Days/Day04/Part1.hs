...
getWinningBoard boards nums = getMinBoardRec ((0,bigFlag),[]) (map (`determineRoundStart` nums) boards)

getMinBoardRec :: ((Int,Int),[[Int]]) -> [((Int,Int),[[Int]])] -> ((Int,Int),[[Int]])
getMinBoardRec board [] = board
getMinBoardRec board1 (board2:boards) 
    | (snd . fst $ board1 ) < (snd . fst $ board2) = getMinBoardRec board1 boards
    | otherwise = getMinBoardRec board2 boards
